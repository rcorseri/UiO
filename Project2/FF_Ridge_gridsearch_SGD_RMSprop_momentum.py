...
#Ridge using stochastic gradient descent with RMSProp and momentum
...
err_train=np.zeros(shape=(lambdas.shape[0],etas.shape[0]))
err_test=np.zeros(shape=(lambdas.shape[0],etas.shape[0]))
    ...
    for eta in etas:
        ...
        for epoch in range(1,n_epochs+1):
            ...
            for minibatch in mini_batches:
                ...
                gradients = (2.0/M)*X_mini.T @ (X_mini @ beta - z_mini) + 2*lmb*beta
...
plt.savefig(f"Results/Ridge/RMSprop/RidgeReg_RMSprop_degree{maxdegree}_train_error.png", dpi=150)
plt.show()
...
plt.savefig(f"Results/Ridge/RMSprop/RidgeReg_RMSprop_degree{maxdegree}_test_error.png", dpi=150)
plt.show()
