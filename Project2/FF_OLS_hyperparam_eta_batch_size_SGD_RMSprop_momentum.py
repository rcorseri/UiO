...
err_train=np.zeros(shape=(M.shape[0],etas.shape[0]))
err_test=np.zeros(shape=(M.shape[0],etas.shape[0]))
...
for i in range(len(M)):
    ...
    for eta in etas:
        ...
        for epoch in range(1,n_epochs+1):
            mini_batches = create_mini_batches(X,z,M[i]) 
            ...
            for minibatch in mini_batches:
                ...
                gradients = (2.0/M[i])*X_mini.T @ (X_mini @ beta - z_mini)
...
heat1.set(xlabel='learning rate', ylabel ='batch size', title = f"Training error \nSGD/RMSprop/momentum")
plt.savefig(f"Results/OLS/RMSprop/OLS_hyper_batch_RMSprop_train_error.png", dpi=150)
plt.show()
...
heat2.set(xlabel='learning rate', ylabel ='batch size', title = f"Test error \nSGD/RMSprop/momentum")
plt.savefig(f"Results/OLS/RMSprop/OLS_hyper_batch_RMSprop_test_error.png", dpi=150)
plt.show()
