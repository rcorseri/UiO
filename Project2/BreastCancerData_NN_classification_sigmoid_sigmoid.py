...
from NeuralNetwork_classification_sigmoid_sigmoid import NeuralNetwork, leakyrelu, leakyrelu_grad, sigmoid, softmax_stable , accuracy_score_numpy
...
n_hidden_neurons = [4,4,4,4,4,4,4,4,4,4]
n_categories = 1
...
for lmbd in lambdas:
    ...
    for eta in etas: 
        ...
        dnn2 = MLPClassifier(hidden_layer_sizes=n_hidden_neurons, activation='logistic', solver ='lbfgs',
...
heat1.set(xlabel='learning rate', ylabel ='regularization', title = f"Accuracy score training set Act:sigmoid (h)-sigmoid (o)")
plt.savefig(f"Results/NN/BreastCancer_sigsig_accuracy_score_train_.png", dpi=150)
plt.show()
...
heat2.set(xlabel='learning rate', ylabel ='regularization', title = f"Accuracy score test set - Act:sigmoid (h)-sigmoid (o)")
plt.savefig(f"Results/NN/BreastCancer_sigsig_accuracy_score_test.png", dpi=150)
plt.show()
...
heat3.set(xlabel='learning rate', ylabel ='regularization', title = f"Accuracy score training set Scikit - Act: sigmoid")
...
heat4.set(xlabel='learning rate', ylabel ='regularization', title = f"Accuracy score test set Scikit- Act: sigmoid")
