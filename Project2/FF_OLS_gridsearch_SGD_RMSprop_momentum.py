...
degrees = np.linspace(1,maxdegree,5)
degrees = degrees.astype(int)

err_train=np.zeros(shape=(degrees.shape[0],etas.shape[0]))
err_test=np.zeros(shape=(degrees.shape[0],etas.shape[0]))

i=0
for degree in degrees:
    j=0
    X = DesignMatrix(x_train[:,0],x_train[:,1], degree)
...
y_axis = degrees # labels for y-axis 
...
heat1.set(xlabel='learning rate', ylabel ='complexity', title = f"Training error \nSGD/RMSprop/momentum")
plt.savefig(f"Results/OLS/RMSprop/OLS_RMSprop_train_error.png", dpi=150)
plt.show()
...
heat2.set(xlabel='learning rate', ylabel ='complexity', title = f"Test error \nSGD/RMSprop/momentum")
plt.savefig(f"Results/OLS/RMSprop/OLS_RMSprop_test_error.png", dpi=150)
plt.show()
